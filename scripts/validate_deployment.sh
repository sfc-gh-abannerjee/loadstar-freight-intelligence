...
check "UDF returns valid score" \
    "SELECT CASE WHEN ${DATABASE}.ML.GET_RECOMMENDATION_SCORE(1, 1) BETWEEN 0.0 AND 1.0 THEN 'VALID' ELSE 'INVALID' END" "VALID"
check "Recommendations table populated" \
    "SELECT CASE WHEN COUNT(*) >= 1000 THEN 'POPULATED' ELSE 'UNDERPOPULATED' END FROM ${DATABASE}.ML.NEXTLOAD_RECOMMENDATIONS" "POPULATED"
